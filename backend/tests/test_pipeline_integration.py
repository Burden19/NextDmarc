...
class FakeImapClient:
    ...
    async def fetch_unseen_messages(self, *, username: str, password: str, mailbox: str):
        ...
        _ = password
        _ = mailbox
        return [ImapMessage(uid="100", raw_message=self.raw_message)]
...
def _build_message_with_dmarc_xml() -> bytes:
    xml = b"""<?xml version="1.0" encoding="UTF-8"?>
<feedback>
    <report_metadata>
        <org_name>Google</org_name>
        <email>noreply-dmarc-support@google.com</email>
        <report_id>integration-r1</report_id>
        <date_range><begin>1712000000</begin><end>1712086400</end></date_range>
    </report_metadata>
...
def test_collect_parse_persist_index_integration(monkeypatch: MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        collect_module,
        "_build_imap_client",
        lambda: FakeImapClient(_build_message_with_dmarc_xml()),
    )
